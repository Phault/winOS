...
export const Minesweeper: FC = () => {
  ...
  const reset = useCallback(() => {
    let difficulty =
      gameOptions.difficulty === Difficulty.Custom
  ...
  const cellMouseDown = useCallback(
    (e: React.MouseEvent, cell: Cell, x: number, y: number) => {
  ...
  const mouseDown = useCallback(
    (e: React.MouseEvent) => {
      if (e.button === 0) setAboutToClick(true);
    },
    [setAboutToClick]
  );

  useGlobalListener('mouseup', e => {
    if (e.button === 0) setAboutToClick(false);
  });

  useLayoutEffect(() => {
    window.rect.width =
      ref.current!.getBoundingClientRect().right - window.rect.left + 5;
    window.rect.height =
      ref.current!.getBoundingClientRect().bottom - window.rect.top + 5;
  ...
  return (
    <Fragment>
        ...
        <MenuBarMenu label="Game">
          ...
          <Item
            onClick={() =>
              setGameOptions(prev => ({
                ...prev,
                unknownMarkEnabled: !prev.unknownMarkEnabled,
              }))
            }>
            Marks (?)
          </Item>
          <Item
            onClick={() =>
              setGameOptions(prev => ({
                ...prev,
                colorEnabled: !prev.colorEnabled,
              }))
            }>
            Color
          </Item>
      ...
      <ThemeProvider
        theme={gameOptions.colorEnabled ? DefaultTheme : ColorlessTheme}>
        <StyledMinesweeper ref={ref} onMouseDown={mouseDown}>
          <Scoreboard>
            <Observer>
              {() => <Counter value={game.mineCount - game.flagsPlaced} />}
            </Observer>
            <Observer>
              {() => {
                let smileyState: SmileyState = aboutToClick
                  ? 'surprised'
                  : 'normal';
            ...
            <AutoRefresh
              interval={200}
              render={() => <Counter value={Math.ceil(game.time / 1000)} />}
            />
          </Scoreboard>
          <Minefield
            grid={game.minefield}
