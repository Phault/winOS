...
export interface NavigationHistoryDropdownProps extends Omit<Toolbar.DropdownProps, 'onClick'> {
    history: string[];
    onClick: (path: string, index: number) => void;
}

export function NavigationHistoryDropdown({history, onClick, children, ...rest}: NavigationHistoryDropdownProps) {
    const items = history.map((val, i) => <Item key={i} onClick={() => onClick(val, i)}>{nodePath.basename(val) || val}</Item>);
...
export function NavigationHistoryButtons({history}: {history: NavigationHistory<string>}) {
    const maxItems = 9;
                    ...
                    return (
                        <Toolbar.SplitButton>
                            ...
                            <NavigationHistoryDropdown 
                                history={previous}
                                onClick={(_path, i) => history.goToIndex(history.position - i - 1)}
                                disabled={!history.canGoBack}>
                                <Separator />
                                <Item>History</Item>
                            </NavigationHistoryDropdown>
