import React, { FC, useContext, useState, useEffect, useCallback, useMemo, useRef, HTMLProps, useReducer, useLayoutEffect, RefObject, MutableRefObject, useImperativeHandle, Ref, CSSProperties, createRef } from "react";
...
export const PictureViewer: FC<PictureViewerProps> = ({ initialPath }) => {
  ...
  const setZoomClamped = useCallback((zoom: number) => {
  ...
  const dir = useDirectory(fileSystem, nodePath.dirname(path), f => supportedFileTypes.test(f));

  const cycleImages = useCallback((offset: number) => {
  ...
  const zoomIn = useCallback(() => {
    if (zoom === 'auto') {
      const containerSize = containerRef.current ? containerRef.current.getBoundingClientRect() : {width: 0, height: 0};
      setZoomClamped(calculateBestFitZoom(imageSize, containerSize) + .2);
    } else {
      setZoomClamped(zoom + .2);
    }
  ...
  const rotateCW = useCallback(() => {
    const rotatedImage = rotateImage(containerRef.current!.firstChild as HTMLImageElement, getMimeType(path));
    writeImage(writeData, rotatedImage);
  }, [containerRef, path, writeImage]);

  const rotateCCW = useCallback(() => {
    const rotatedImage = rotateImage(containerRef.current!.firstChild as HTMLImageElement, getMimeType(path), true);
    writeImage(writeData, rotatedImage);
  }, [containerRef, path, writeImage]);

  const onWheel = useCallback((e: React.WheelEvent<any>) => {
    if (e.deltaY < 0)
        zoomIn();
    else
        zoomOut();
  ...
  const actions = useMemo(() => preventDefaultAll({
  ...
  return (
    <HotKeys keyMap={KeyboardShortcuts} handlers={actions} component={StyledPictureViewer} onWheel={onWheel} allowChanges>
        ...
        <Toolbar>
            ...
            <Button onClick={nextImage}><Icon src={assets.next} /></Button>
            <Separator />
            <Button onClick={setBestFit}><Icon src={assets.bestFit} /></Button>
            <Button onClick={setActualSize}><Icon src={assets.actualSize} /></Button>
            <Button disabled><Icon src={assets.slideshow} /></Button>
            <Separator />
            <Button onClick={zoomIn}><Icon src={assets.zoomIn} /></Button>
            <Button onClick={zoomOut}><Icon src={assets.zoomOut} /></Button>
            <Separator />
            <Button onClick={rotateCW}><Icon src={assets.rotateRight} /></Button>
            <Button onClick={rotateCCW}><Icon src={assets.rotateLeft} /></Button>
            <Separator />
            <Button disabled><Icon src={assets.remove} /></Button>
            <Button disabled><Icon src={assets.print} /></Button>
            <Button disabled><Icon src={assets.save} /></Button>
            <Button disabled><Icon src={assets.edit} /></Button>
            <Separator />
            <Button disabled><Icon src={assets.help} /></Button>
        </Toolbar>
    </HotKeys>
  );
};
