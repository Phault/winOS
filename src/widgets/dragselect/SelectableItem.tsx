...
export const SelectableItem = <T extends {}>({
  children,
  data,
...
export function asSelectable<P, D = any>(
  WrappedComponent: React.ComponentType<P & SelectableItemChildProps>
): FC<P & SelectableItemProps<D>> {
  return ({ data, ...rest }: P & SelectableItemProps<D>) => (
    <SelectableItem data={data}>
      {selectableProps => (
        <WrappedComponent {...(rest as P)} {...selectableProps} />
      )}
    </SelectableItem>
  );
}
