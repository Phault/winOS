import React, { FC, Ref, useState, useCallback, useEffect, useMemo } from 'react';
...
export interface SelectableGroupProps {
    component?: React.ComponentType<SelectableWrappedRequiredProps> | 'div';
}

export const SelectableGroup: FC<SelectableGroupProps> = ({component: Component = 'div', children, ...rest}) => {
    ...
    const rect = useMemo(() => {
        if (!start || !stop || !ref.current)
            return null;

        const left = Math.max(0, Math.min(start.left, stop.left));
        const top = Math.max(0, Math.min(start.top, stop.top));
        const right = Math.min(ref.current.scrollWidth, Math.max(start.left, stop.left));
        const bottom = Math.min(ref.current.scrollHeight, Math.max(start.top, stop.top));
    ...
    return (
        <SelectionContext.Provider value={context}>
            <Component {...rest} ref={ref} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp}>
                {children}
...
export function asSelectableGroup<P>(WrappedComponent: React.ComponentType<P>, component?: React.ComponentType<SelectableWrappedRequiredProps>): FC<P> {
    return props => <SelectableGroup component={component}><WrappedComponent {...props} /></SelectableGroup>;
}