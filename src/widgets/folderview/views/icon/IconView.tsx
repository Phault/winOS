...
export const IconView: React.FC<ViewModeProps> = ({ files, onExecute }) => {
  ...
  function onFileClick(e: React.MouseEvent, file: FileInfo) {
    const [element] = [...selection.items].find(
      ([_element, config]) => config.data === file
    )!;

    if (e.ctrlKey) selection.toggleSelected(element);
    else selection.setSelection([element]);

    e.stopPropagation();
  }

  function onFileDoubleClick(e: React.MouseEvent, file: FileInfo) {
    if (onExecute) onExecute([file]);
  ...
  return (
    <>
      <StyledIconView
        onMouseDown={e => !e.ctrlKey && selection.clearSelection()}>
        {(files || []).map(f => (
          <MenuProvider
