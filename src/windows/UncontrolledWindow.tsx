...
export interface UncontrolledWindowProps
  extends Partial<WindowProps & ResizableProps & MovableProps> {}
...
const UncontrolledWindow: React.FC<UncontrolledWindowProps> = props => {
  ...
  const onMove = useCallback(
    (pos: Position) => {
      ...
      if (props.onMove) props.onMove(pos);
    },
  ...
  const onResize = useCallback(
    (bounds: Rectangle) => {
      ...
      if (props.onResize) props.onResize(bounds);
    },
    [setRect, props.onResize]
  );

  return <Window {...props} {...rect} onMove={onMove} onResize={onResize} />;
};
