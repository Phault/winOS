...
export function getDefaultIcon({path, stats}: FileInfo) {
    ...
    if (stats.isDirectory()) {
        ...
        switch (nodePath.basename(path)) {
            case 'My Documents': return folderDocumentsIcon;
            case 'My Pictures': return folderPicturesIcon;
            case 'My Music': return folderMusicIcon;
            case 'Recent': return folderRecentIcon;
            case 'Favorites': return folderFavoritesIcon;
        }
    ...
    switch (ext) {
        ...
        case '.sys':
        case '.ini': return configIcon;
    }

    return defaultIcon;
}

export function createNewFolder(fs: FSModule, parentPath: string, folderName = 'New Folder') {
